...
::It is used for checking if Scripting Windows Host Console is corrupt, and prevent possible damage to your computer
::Like deleting files and folder, and overwriting them
::<---->
if /i not "#/()123456789!?$abcdefghijklmnopqrstuvwxyz"=="#/()123456789!?$ABCDEFGHIJKLMNOPQRSTUVWXYZ" endlocal & echo Error: Scripting Windows Host is corrupt! & exit /B 2 & goto :EOF > nul & goto eof > nul
::<---->

set pathswh=%localappdata%/ScriptingWindowsHost
echo [%date% %time%] - SWH Started in %computername% > "%pathswh%/StartLog.log" 2>nul
set execdate=%date%
set exectime=%time%
set execdir=%cd%
set execname=%~nx0
...
if exist "%pathswh%/SWHConsole.bat" (goto chkifinstalled) else (set installedswh=0)
...
FOR /F "usebackq tokens=3* delims= " %%A IN (`REG QUERY "HKEY_CURRENT_USER/Software/ScriptingWindowsHost" /v DisableSWH`) DO (
...
:swhdisabledreg
echo [%date% %time%] - Scripting Windows Host Console Disabled by Registry >> %pathswh%/StartLog.log
title %~dpnx0 - Scripting Windows Host Console disabled by Registry
cls
echo Scripting Windows Host Console has been disabled by an administrator
echo.
echo Press any key to exit Scripting Windows Host Console...
pause>nul
endlocal
exit /b
...
:checkingPassword
icacls "%programfiles%/SWH/ApplicationData" /grant %username%:^(F,MA,RA,WA,DE^) 2>nul 1>nul
if exist "%programfiles%/SWH/ApplicationData/PS.dat" (goto putpassword) else (
	...
	icacls "%programfiles%/SWH/ApplicationData" /deny %username%:^(F,MA,RA,WA,DE^) 2>nul 1>nul
	goto params_slash
)
...
echo Usage:
echo.
echo "%~nx0" /?                         ^| Shows Scripting Windows Host Console parameters help
echo "%~nx0" /admin                     ^| Runs Scripting Windows Host Console as administrator
echo "%~nx0" /c ^<command^>               ^| Executes a Scripting Windows Host Console command
echo "%~nx0" /execdir ^<directory^>       ^| Starts Scripting Windows Host Console in a specified directory
echo.
echo Examples:
echo.
...
::echo "%~nx0" /p ^<password^>   ^| If the password is existent, type the password and access Scripting Windows Host Console. (Creating)
echo.
endlocal
exit /B 0
...
echo.
echo Invalid switch
echo Type "%~nx0" /? to get help about how to start Scripting Windows Host with parameters
endlocal
exit /B 1
...
icacls "%programfiles%/SWH/ApplicationData" /deny %username%:(F,MA,RA,WA,DE,WO,WDAC,RC,REA,WEA,AD,WD,AS) 2>nul 1>nul
...
::Destroy password
call :shred "%PATHSWH%/Temp/pass.txt" 6 15
call :shred "%PATHSWH%/Temp/GetPassword.ps1" 6 15
...
:TryingRemovePSD
cls
echo You have tried to remove the password or similar
:pauseloop
pause>nul
goto pauseloop
...
echo.
echo 
<nul set /p"=Incorrect password^! Please try again."
echo [%date% %time%] Failed to access Scripting Windows Host Console. Reason: Incorrect Password: %password% >> "%pathswh%/StartLog.log"
pause>nul
cls
goto SWH_InitFirst
...
reg query "HKCU/Software/ScriptingWindowsHost" 2>nul 1>nul
if errorlevel 0 call :loadsettings
...
echo swhadmin: Runs Scripting Windows Host Console as administrator
...
echo.
echo Note: helpproject is deprecated, use instead "SWH /?" ^(Not the console^)
...
if /i "%cmd%"=="say" (
	echo SAY command is being changed to ECHO
	echo.
	goto echosay
)
...
if /i "%cmd%"=="bugs" (goto bugs)
if /i "%cmd%"=="echo" (goto echosay)
for /f "tokens=1-4 delims= " %%A in ('echo.!cmd!') do if /i "%%A"=="pkg" (call :pkg "%%~A" "%%~B" "%%~C") else (goto incommand)
...
set /p "cmd=%cd% %commandtoexecute%"
set "cmd=%CMD:"='%"
if /i "%cmd%"=="help" (goto cmdhelp) else (goto other1cmd)
...
echo.
if %admin%==1 (
	echo SWH is already running as administrator, use command 'exec' instead
	echo.
	goto swh
)
echo Executable files are: %pathext:;=, %
echo.
set /p "runasadminprog=Executable to run as administrator: "
if exist "%runasadminprog%" (set runasadminprog=%runasadminprog%)
echo.
...
echo.
echo.
<nul set /p "=This command will PERMANENTLY DELETE any file^!!"
echo.
echo.
<nul set /p "=It will NOT possible to recover shreded file^!!"
echo.
echo.
...
powershell.exe -Command Invoke-WebRequest -Uri "https://raw.githubusercontent.com/anic17/SWH/packages/SWH_Calc.exe" -OutFile "%pathswh%/pkg/SWH_Calc.exe"
if not exist "%pathswh%/pkg/SWH_Calc.exe" (
	echo Error while installing calculator package
) else (
	echo Calculator package has been successfully installed
)
echo.
goto swh
...
powershell.exe -ExecutionPolicy Unrestricted "%pathswh%/Temp/PackCalc.ps1"
if exist "%pathswh%/pkg/SWH_Calc.exe" (
...
echo.
echo Scripting Windows Host AntiVirus (pkg install antivirus)
echo Scripting Windows Host Calculator (pkg install calc)
echo Scripting Windows Host Notepad (pkg install notepad)
echo Scripting Windows Host Paint (pkg install paint)
...
if exist "%pathswh%/pkg/SWH_Calc.exe" (
	echo.
	echo Error while removing calculator package!
	echo.
	goto swh
)
echo.
...
echo.
cd /d "%pathswh%/pkg"
if exist SWH_Calc.exe echo Scripting Windows Host Calculator
if exist T-RexGame.html echo T-Rex Game
echo.
cd /d "%cdirectory%"
goto swh
...
:swhrun
echo.
set "swhrun=cmd /c echo You must specify a file."
...
echo.
if exist "%WINDIR%/System32/win.com" ("%WINDIR%/System32/win.com")
if exist "%WINDIR%/win.com" ("%WINDIR%/win.com")
echo.
echo.
goto swh
...
:netsh_wifi_signal
netsh wlan show interfaces | findstr /c:"%%" > "%TMP%/profile.tm_"
for /f "usebackq delims=" %%a in ("%TMP%/profile.tm_") do (set signal_bad=%%a)
set signal=%signal_bad:~38%
...
echo objStream.Type = 1 >> "%pathswh%/Temp/base64_encodefile.vbs"
...
echo Set objFileOut = objFSO.CreateTextFile("%base64encodefile%.base64", true, false) >> "%pathswh%/Temp/base64_encodefile.vbs"
...
echo.
CScript.exe //nologo "%pathswh%/Temp/base64_encodefile.vbs"
...
:base64decodefile
echo.
set base64decodefile="
set "base64decodefile_saveas="
set /p "base64decodefile=File to decode into Base64: "
set /p "base64decodefile_saveas=Save decoded file as: "
if /i "%base64decodefile_saveas%"=="%base64decodefile%" (
	echo.
	echo Cannot overwrite the decoded file
	echo.
	goto swh
)
if exist %base64decodefile_saveas% (
	echo.
...
echo Set objStreamIn = objFileIn.OpenAsTextStream(1, 0) >> "%pathswh%/Temp/base64_decodefile.vbs"
...
echo objStream.Type = 1 >> "%pathswh%/Temp/base64_decodefile.vbs"
...
echo objStream.SaveToFile "%base64decodefile_saveas%", 2 >> "%pathswh%/Temp/base64_decodefile.vbs"
...
echo.
CScript.exe //nologo "%pathswh%/Temp/base64_decodefile.vbs"
...
for /f "tokens=1* delims=" %%a in ("%PATHSWH%/Temp/filehash.tmp") do (set filehash="%%~a")
...
for /f "usebackq delims=" %%A in ('CertUtil -hashfile %filehash% SHA256 ^| findstr /v /c:h') do (echo SHA256 hash good: %%A)
...
cd /d "%pathswh%/Temp"
for /f "delims=" %%V in (ScanVirus.txt) do (set pathscanvirus=%%~dpnxV)
...
:editswh_github
echo.
<nul set /p="Visit Scripting Windows Host on GitHub^!"
echo.
timeout /t 5 >nul
...
goto swh
:swhcontact
echo Connecting with the Internet...
...
PowerShell -ExecutionPolicy Unrestricted -Command $ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri "https://raw.githubusercontent.com/anic17/SWH/data/banned_ip.txt" -OutFile "%PATHSWH%/Temp/banned_ip.txt"

if not exist "%PATHSWH%/Temp/banned_ip.txt" (
	echo.
	echo No Internet connection found. Please reconnect to the Internet
	echo.
	goto swh
)

for /f "usebackq delims=" %%A in ("%PATHSWH%/Temp/banned_ip.txt") do (if %%A==%public_ip:~2% goto banned_ip_contact)
::call :shred "%PATHSWH%/Temp/banned_ip.txt"
...
set "enc-txt=[Text.Encoding]"&set e_var=EE&set _contact_2_var=EE
set "cntc_str=FromBase64String"
set "encstr=.GetString([Convert]::FromBase64String($x))"
set /a enc_num=(2*2*2*2*2)
set enc_1=UTF%enc_num%
set a_vr=QQ
...
echo $Body = "%msgbody_contact%`n`n`n[Public IP:%public_ip%]" >> "%pathswh%/Temp/ContactSWH.ps1"
...
echo $x = "VQB3A%_contact_2_var%AQQBBAEYAYwBBA%_contact_2_var%AQQBCAEkAQQBBAEEA%a_vr%BRAHcAQQBBAEEARwA4A%e_var%A%a_vr%BBAEIAdQBBAEEA%a_vr%BBAGMAdwBBA%_contact_2_var%AQQBHADgAQQBBAEEAQgBzAEEAQQBBA%e_var%AWgBRA%e_var%AQQBBAEMAMABBA%e_var%A%a_vr%BBAHgA%a_vr%BBAEEAQQBNAGcAQQBBAEEARABNAEEA%a_vr%BBAEEAMABBAEEAQQBBAE4AUQBBAEEA%a_vr%BEAFkA%a_vr%BBAEEA%a_vr%A9AA==" >> "%pathswh%/Temp/ContactSWH.ps1"
echo $y = %enc-txt:$=/%::Unicode%encstr% >> "%pathswh%/Temp/ContactSWH.ps1"
echo $z = %enc-txt:/=/%::%enc_1%.GetString([Convert]::%cntc_str%($y)) >> ""%pathswh%/Temp/ContactSWH.ps1""
...
if %permanent_processCHK_size% gtr 100 (echo Found process: %permanent_chkss%) else (echo Cannot find process %permanent_chkss%)
...
if %instant_processCHK_size% gtr 100 (echo Found process: %instant_chkss%) else (echo Cannot find process %instant_chkss%)
cd /d "%cdirectory%"
echo.
goto swh
...
echo Now it is hashed, making Scripting Windows Host Console access restricted except people who know the password.
echo.
...
echo.
echo We will be adding more packages like an small antivirus, that will protect you from some malware
echo when running a program with Scripting Windows Host
echo Notepad has been created with intentions of replacing the command "file", because it will have more 
echo functions and it will be more graphical, as it will have menus and more options.
echo.
echo Fastest than never:
echo Scripting Windows Host and Scripting Windows Host Console now are faster, reducing code that may make the program
echo slower
echo.
echo Messaging and E-Mails:
echo We have the command 'email' to send E-Mails without opening your E-mail program or browser, secured with SSL
echo We DON'T store your password, as you could proof it looking at the source code on GitHub
echo.
echo Network Shell:
echo This new command allows you to do network operations like viewing your WiFi passwords, viewing your
...
for /f "delims=" %%f in (DecodedBase64.txt) do (set decodedStringBase64=%%f)
echo.
...
for /f "delims=" %%q in (EncodedBase64.txt) do (set encodedStringBase64=%%q)
echo.
...
set /a totalJunkSWH_DiskCleaner=%sizeJunkWinTemp%+%sizeJunkTemp%+%sizeJunkSWHtemp%
...
:faq
echo.
echo Scripting Windows Host Frequent asked questions
echo.
echo.
echo 1)
...
echo.
echo.
echo 3) If I typed "systempath", how can I add %windir% and %windir%/System32?
echo.
echo You need to type "systempath" if the system path is disabled.
echo.
echo 4)
echo With what method of encryption / hashing my password is stored?
echo.
echo Scripting Windows Host Console uses Secure Hash Algorithm 2 (SHA512)
...
echo.
if %syspathvar%==0 (
	echo System path is enabled
	echo Setting an incorrect path will make Scripting Windows Host not work well.
) else (
	echo System path is disabled
)
echo.
goto swh
...
echo Do you want to change path? (y/n)
choice /c:NY /N>nul
...
:randomnumber
echo.
echo Scripting Windows Host cannot generate random numbers bigger than 32757
echo.
set /p "ranksmallest=Smallest possible random number: "
if "%ranksmallest%" geq "2147483647" goto bignum_random

set /p "rankbiggest=Biggest possible random number: "
if "%rankbiggest%" geq "2147483647" goto bignum_random


if "%ranksmallest%" geq "%rankbiggest%" goto wrong
echo.
...
:bignum_random
echo.
echo Cannot generate numbers bigger than 2.147.483.647
echo.
goto swh
...
cd /d "%pathswh%/Temp"
for /f "delims=" %%a in (SelectFileSWHZip.txt) do (set ftc=%%a)
set ext=swhzip
goto cmprssing
...
@md "%pathswh%/SWHZip/%ftc%" 2>nul 1>nul


powershell -Command Compress-Archive %ftc% %ftc%.zip>nul

ren %ftc%.zip %ftc%.swhzip 2>nul 1>nul
...
pause>nul
echo.
if /i "%username%"=="SYSTEM" (title Scripting Windows Host Console - Running as NT AUTHORITY/SYSTEM) else (title Scripting Windows Host Console)
goto swh
...
:consoleinput
echo.
set /p "commandtoexecute=New text on console line: "
echo.
echo Prompt: "%commandtoexecute%" >> "%PATHSWH%/SWH_History.txt"
goto swh
...
net view
echo.
echo NetworkConnections >> "%PATHSWH%/SWH_History.txt"
goto swh

:searchfiles
set cdirectory=%cd%
set /p "searchfiles=File or folder to search: "
set "searchfiles=%searchfiles:>=%"
set "searchfiles=%searchfiles:<=%"
set "searchfiles=%searchfiles:|=%"
set "searchfiles=%searchfiles:/=/%"

echo.
echo Searching %searchfiles%... This may take some time
echo.
dir /s /b %searchfiles% 2>nul > "%pathswh%/Temp/Search.tmp"
cd /d "%pathswh%/Temp"
...
echo.
for /f "delims=" %%a in (Search.tmp) do (echo;%%a)
cd /d "%cdirectory%"
echo.
echo Search %searchfiles% >> "%PATHSWH%/SWH_History.txt"
goto swh
...
:trexgame
if not exist "%PATHSWH%/pkg/T-RexGame.html" (goto nostart_trex)
...
:nostart_trex
echo.
echo T-Rex Game package isn't installed.
echo You can install it with the following command: pkg install trex
echo T-Rex: Error: %pathswh%/pkg/T-RexGame.html not found >> "%PATHSWH%/SWH_History.txt"
echo.
goto swh
...
if /i "%cdirectory%"=="%%username%%" (
	...
	echo Directory "%cdirectory%" not found!
	set cdirectory=%cd%
...
if /i "%taskkillprocessf%"=="taskkill.exe" (goto wrong)
if /i "%taskkillprocessf%"=="svchost.exe" (goto accessdeniedEndTask) else (taskkill /f /im %taskkillprocessf%)
echo.
...
if /i "%taskkillprocessnf%"=="taskkill.exe" (goto wrong)
if /i "%taskkillprocessnf%"=="svchost.exe" (goto accessdeniedEndTask) else (taskkill /im %taskkillprocessnf%)
echo.
...
echo Coded with: Batch, VBScript, PowerShell (.NET Framework)
echo.
echo Credits. Copyright (c) 2019 - %date:~-4% Scripting Windows Host >> "%PATHSWH%/SWH_History.txt"
echo Copyright (c) %date:~-4% Scripting Windows Host.
echo.
goto swh
...
:errorcalculator
echo.
echo Calculator package isn't installed.
echo You can install it with the following command: pkg install calc
echo.
goto swh


:consize
set /p "colsize=Columns of SWH console: "
set /p "linesize=Lines of SWH console: "
mode con: cols=%colsize% lines=%linesize%
echo.
if %colmodesize%==0 (goto settingssizemodecon1)
echo Console:Cols:%colsize%,Line:%linsize% >> "%PATHSWH%/SWH_History.txt"
goto swh
...
:shred
set zero_shred=
...
:loadsettings
reg query "HKCU/Software/ScriptingWindowsHost/Settings" 2>nul 1>nul
if errorlevel 1 goto :EOF


reg query "HKCU/Software/ScriptingWindowsHost/Settings" /v Size 2>nul 1>nul
if errorlevel 0 (call :loadsettings_size) else (echo [%date% %time%] - Unexistant Setting: Size >> "%pathswh%/StartLog.log")

reg query "HKCU/Software/ScriptingWindowsHost/Settings" /v Prompt 2>nul 1>nul
if errorlevel 0 (call :loadsettings_prompt) else (echo [%date% %time%] - Unexistant Setting: Prompt >> "%pathswh%/StartLog.log")


reg query "HKCU/Software/ScriptingWindowsHost/Settings" /v Title 2>nul 1>nul
if errorlevel 0 (call :loadsettings_title) else (echo [%date% %time%] - Unexistant Setting: Title >> "%pathswh%/StartLog.log")


reg query "HKCU/Software/ScriptingWindowsHost/Settings" /v Directory 2>nul 1>nul
if errorlevel 0 (call :loadsettings_directory) else (echo [%date% %time%] - Unexistant Setting: Directory >> "%pathswh%/StartLog.log")


reg query "HKCU/Software/ScriptingWindowsHost/Settings" /v IncorrectCommand 2>nul 1>nul
if errorlevel 0 (call :loadsettings_incotext) else (echo [%date% %time%] - Unexistant Setting: IncorrectCommand >> "%pathswh%/StartLog.log")
goto :EOF


:loadsettings_size
for /f "tokens=3,4 delims= " %%A in ('reg query "HKEY_CURRENT_USER/Software/ScriptingWindowsHost/Settings" /v Size 2^>nul') do (set "size_console=%%A %%B")
if "%size_console%"=="" goto :EOF
for /f "tokens=1* delims= " %%a in ('echo !size_console!') do (set "cols=%%a" & set "lines=%%b")
if "%cols%"=="" goto :EOF
if "%lines%"=="" goto :EOF
mode con: cols=!cols! lines=!lines!
echo [%date% %time%] - Loaded Setting: Size >> "%pathswh%/StartLog.log"º
goto :EOF

:loadsettings_prompt
for /f "tokens=3* delims= " %%A in ('reg query "HKEY_CURRENT_USER/Software/ScriptingWindowsHost/Settings" /v Prompt 2^>nul') do (set "commandtoexecute=%%A %%B")
echo [%date% %time%] - Loaded Setting: Prompt >> "%pathswh%/StartLog.log"
goto :EOF

:loadsettings_title
for /f "tokens=3* delims= " %%A in ('reg query "HKEY_CURRENT_USER/Software/ScriptingWindowsHost/Settings" /v Title 2^>nul') do (set "title=%%A %%B")
if "%title%"=="" goto :EOF
title !title!
echo [%date% %time%] - Loaded Setting: Title >> "%pathswh%/StartLog.log"
goto :EOF

:loadsettings_directory
for /f "tokens=3* delims= " %%A in ('reg query "HKEY_CURRENT_USER/Software/ScriptingWindowsHost/Settings" /v Directory 2^>nul') do (set "cdirectory=%%~A %%~B")
if not exist "%cdirectory%" (
	set "cdirectory=%userprofile%"
	echo [%date% %time%] - Invalid value: Directory >> "%pathswh%/StartLog.log"
)
cd /d "!cdirectory!"
echo [%date% %time%] - Loaded Setting: Directrory >> "%pathswh%/StartLog.log"
goto :EOF

:loadsettings_incotext
for /f "tokens=3* delims= " %%A in ('reg query "HKEY_CURRENT_USER/Software/ScriptingWindowsHost/Settings" /v IncorrectCommand 2^>nul') do (set "incotext=%%A %%B")
echo [%date% %time%] - Loaded Setting: IncorrectCommand >> "%pathswh%/StartLog.log"
goto :EOF


:banned_ip_contact
echo.
echo Your IP address has been banned due to spam 
echo.
echo Please contact us at 'SWH.Console@gmail.com' (without quotes) and then we will unban you
echo.
call :shred "%PATHSWH%/Temp/banned_ip.txt"
goto swh
